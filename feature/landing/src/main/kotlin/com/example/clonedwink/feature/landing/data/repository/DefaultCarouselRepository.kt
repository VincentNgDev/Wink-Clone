...
// data source today; a real app would instead take a `core:network` Retrofit service here.
//
...
// (see di/LandingModule.kt) without risking an Activity-context leak.
class DefaultCarouselRepository @Inject constructor(
    ...
    // no real artwork yet, so feature:landing's LandingScreen.kt's GlassSlideCard falls back to
    // a brand-color gradient keyed by `id` instead. A real photo-backed slide (imageUrl from a
